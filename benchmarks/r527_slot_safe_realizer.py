...
SEED=20260810
# Preserve the R5.12 128-token physical context exactly: 56 plan tokens + start + 71 previous output tokens.
PREFIX_TOKENS=56
TARGET_TOKENS=72
BATCH=8
END_MARK='<END>'
...
TEMPLATES={
 ...
 'temporal':{
   ...
   'формально':'По состоянию на [S2] для сущности «[S0]» отношение «[S1]» имеет значение «[S3]». Источник: [S4].',
 ...
 'control':{
   'кратко':'Цель [S0], текущее [S1], ошибка [S2]. Допуск [S3]. Режим [S4]. Действие: [S5].',
...
def abstract_case(plan,style):
    ...
    # Keep the routing variables at the RIGHT edge because the model only sees the final PREFIX_TOKENS pieces.
    prompt=(
        'СЛОТЫ: '+', '.join(f'[S{i}]={k}' for i,k in enumerate(keys))+'. '
        'Значения слотов не генерируй; маркеры [S0], [S1] и остальные сохрани буквально. '
        f'ШАГИ={len(plan.steps)}; ТИП={key}; СТИЛЬ={style}; ОТВЕТ:'
    )
    return prompt,TEMPLATES[key][style]+' '+END_MARK,slots,key

def encode_example(tok,prompt,target):
    c=tok.enc(prompt.encode('utf-8'))[-PREFIX_TOKENS:];c=[0]*(PREFIX_TOKENS-len(c))+c
    ...
    assert len(x)==128 and len(y)==128 and len(m)==128
    return x,y,m
...
def train(model,tok,steps):
    rng=random.Random(SEED+5271);torch.manual_seed(SEED);opt=torch.optim.AdamW(model.parameters(),lr=2e-4,betas=(.9,.95),weight_decay=.01);hist=[];t0=time.perf_counter();seen=0;model.train()
    for step in range(steps):
        xs=[];ys=[];ms=[]
        for _ in range(BATCH):
            plan,_=make_case(rng,rng.choice(KINDS));p,t,_,_=abstract_case(plan,rng.choice(STYLES));x,y,m=encode_example(tok,p,t);xs.append(x);ys.append(y);ms.append(m);seen+=1
        ...
        opt.zero_grad(set_to_none=True);loss.backward();torch.nn.utils.clip_grad_norm_(model.parameters(),1.0);opt.step();hist.append(float(loss.detach()))
...
def batch_generate(model,tok,prompts,max_new=TARGET_TOKENS):
    ...
    outs=[[] for _ in prompts];model.eval();done=[False]*len(prompts)
    with torch.no_grad():
        for _ in range(max_new):
            seq=[contexts[i]+[0]+outs[i] for i in range(len(prompts))]
            # Generation can exceed 128 if left unchecked; retain the current physical context window.
            seq=[s[-128:] for s in seq]
            x=torch.tensor(seq,dtype=torch.long);z=model(x);nxt=z[:,-1].argmax(-1).tolist()
            for i,n in enumerate(nxt):
                if not done[i]:outs[i].append(int(n))
            for i,o in enumerate(outs):
                if END_MARK in tok.dec(o).decode('utf-8','replace'):done[i]=True
            if all(done):break
    texts=[]
    for o in outs:
...
def skeleton_ok(text,key):
    expected={str(i) for i in range(len(SLOT_KEYS[key]))};got=set(SLOT_RE.findall(text))
    if got!=expected:return False,'slot_set'
    if '�' in text:return False,'utf8'
    if len(text)<12 or len(text)>700:return False,'length'
    ...
    if key=='conflict_blocked' and not any(x in text.lower() for x in ('заблок','нельзя','запрещ','противореч')):return False,'conflict_status'
    return True,'ok'

def bind(text,slots):
    for k,v in slots.items():text=text.replace(k,str(v))
    return text

def evaluate(model,tok,n=4096):
    rng=random.Random(SEED+9271);cases=[]
    for _ in range(n):
        plan,truth=make_case(rng,rng.choice(KINDS));style=rng.choice(STYLES);p,t,slots,key=abstract_case(plan,style);cases.append((plan,truth,style,p,t,slots,key))
    neural_ok=0;final_ok=0;fallback=0;reasons=Counter();per=defaultdict(lambda:{'n':0,'accepted':0,'verified':0,'fallback':0});rows=[]
    for s in range(0,n,64):
        batch=cases[s:s+64];texts=batch_generate(model,tok,[x[3] for x in batch])
        for item,text in zip(batch,texts):
            plan,truth,style,p,target,slots,key=item;per[key]['n']+=1;ok,reason=skeleton_ok(text,key);neural_ok+=int(ok);per[key]['accepted']+=int(ok);reasons[reason]+=1
            if ok:
                bound=bind(text,slots);good=verify_bound(plan,truth,bound)
                if good:final=bound
                else:fallback+=1;per[key]['fallback']+=1;final=deterministic_render(plan);good=verify_bound(plan,truth,final)
            else:
                fallback+=1;per[key]['fallback']+=1;final=deterministic_render(plan);good=verify_bound(plan,truth,final)
            final_ok+=int(good);per[key]['verified']+=int(good)
            if len(rows)<120:rows.append({'task':key,'style':style,'neural_skeleton':text,'skeleton_ok':ok,'reason':reason,'slots':slots,'final':final,'final_verified':good,'teacher_skeleton':target.replace(' '+END_MARK,'')})
    for v in per.values():
        v['accept_rate']=v['accepted']/v['n'];v['verified_rate']=v['verified']/v['n'];v['fallback_rate']=v['fallback']/v['n']
    return {'n':n,'neural_skeleton_accept':neural_ok/n,'final_verified_accuracy':final_ok/n,'fallback_rate':fallback/n,'reasons':dict(reasons),'per_task':dict(per),'rows':rows}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--tokenizer-model',required=True);ap.add_argument('--warm-checkpoint',required=True);ap.add_argument('--steps',type=int,default=4096);ap.add_argument('--eval-n',type=int,default=4096);ap.add_argument('--out',default='nexus_r5271_results');ap.add_argument('--threads',type=int,default=2);a=ap.parse_args();torch.set_num_threads(a.threads);out=Path(a.out);out.mkdir(parents=True,exist_ok=True);shutil.copy2(a.tokenizer_model,out/'sp_unigram_4096.model');tok=FixedSP(a.tokenizer_model);m=load_warm(a.warm_checkpoint);tr=train(m,tok,a.steps);ev=evaluate(m,tok,a.eval_n)
    result={'format':'nexus-r5271-slot-safe-neural-realizer/2','protocol':{'params':base.param_count(m),'warm_start':'R5.12 32K Russian organ','context_split':{'plan_tokens':PREFIX_TOKENS,'surface_tokens':TARGET_TOKENS},'task_control':'task/status key moved to right edge immediately before ОТВЕТ','slot_policy':'neural model never receives exact slot values; it emits only [S0]...[Sn]; Authority binds exact values afterward','fallback':'deterministic verified renderer on malformed skeleton or failed semantic verification','steps':a.steps,'train_examples':tr['examples'],'eval_unseen':a.eval_n},'training':tr,'evaluation':{k:v for k,v in ev.items() if k!='rows'}}
    (out/'00_R5271_RESULTS.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');(out/'01_EXAMPLES.json').write_text(json.dumps(ev['rows'],ensure_ascii=False,indent=2),encoding='utf-8');torch.save({'state_dict':m.state_dict(),'protocol':result['protocol']},out/'R5271_SLOT_SAFE_REALIZER_3M.pt');print(json.dumps(result,ensure_ascii=False),flush=True)
