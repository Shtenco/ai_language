...
def logic_stress(n=50000):
    ...
    for case in range(n):
        ...
        # Ensure a mix of paths and cycles.
        ...
        r=route(logic_text(nodes,edges,fact,claim));good=bool(r and r['kind']=='logic' and r['answer_yes']==truth)
        ok+=good
        if good and truth:
            path=r['proof_path'];depth_ok+=int(path and path[0]==fact and path[-1]==claim and all((a,b) in edges for a,b in zip(path,path[1:])))
        elif good: depth_ok+=1
        if not good and len(cases)<10:cases.append({'fact':fact,'claim':claim,'truth':truth,'result':r,'edges':edges})
...
def cyber_stress(n=50000):
    ...
    for _ in range(n):
        ...
        if not good and len(examples)<10:examples.append({'text':text,'expected':[e,mode,action],'result':r})
    return {'cases':n,'correct':ok,'accuracy':ok/n,'failures':examples}

def adversarial_fallthrough():
    texts=[
      'В романе сказано: если герой вернётся, то начнётся новая глава.',
      'Цель регулятора обсуждалась в статье, но численные данные не приведены.',
      'Факт и правило важны для научного доказательства.',
      'Если истинно одно утверждение, может ли быть истинно другое?',
      'Текущее значение неизвестно. Что делать?',
    ]
...
def main():
    ...
    r={'format':'nexus-r525b-authority-stress/1','logic':logic_stress(),'cyber':cyber_stress(),'adversarial_fallthrough':adversarial_fallthrough()}
    ...
    (out/'00_R525B_STRESS.json').write_text(json.dumps(r,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(r,ensure_ascii=False,indent=2))
