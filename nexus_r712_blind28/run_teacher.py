...
EXPECTED='25ae7885469162214fa8235b06ef2a71e87890f7811bc3dd58538ffb87a888b6'
here=Path(__file__).parent
packed=''.join((here/f'b28_chunk_{i}.txt').read_text(encoding='utf-8') for i in range(4))
raw=zlib.decompress(base64.b85decode(packed.encode('ascii')))
bsha=hashlib.sha256(raw).hexdigest()
if bsha!=EXPECTED: raise SystemExit(f'BLIND28 HASH MISMATCH {bsha} != {EXPECTED}')
bpath=here/'BLIND28_FROZEN.json'; bpath.write_bytes(raw); bench=json.loads(raw.decode('utf-8'))
...
for it in bench['items']:
    ...
    body={'model':MODEL,'messages':[{'role':'system','content':system},{'role':'user','content':prompt}],
          ...
          'options':{'temperature':0,'seed':280712,'num_predict':8,'top_p':1.0}}
    t=time.time()
...
summary={'model':MODEL,'model_id':MODEL_ID,'blind28_sha256':bsha,'n':len(rows),'valid':len(valid),
