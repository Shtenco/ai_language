...
BENCH=Path(__file__).with_name('BLIND24_FROZEN.json'); BASE='http://127.0.0.1:11434'
...
def option_lp(resp):
    ...
    return best
bench=json.loads(BENCH.read_text(encoding='utf-8')); bsha=hashlib.sha256(BENCH.read_bytes()).hexdigest()
...
for it in bench['items']:
    ...
    body={'model':MODEL,'messages':[{'role':'system','content':system},{'role':'user','content':prompt}],
          ...
          'options':{'temperature':0,'seed':24077,'num_predict':8,'top_p':1.0}}
    t=time.time()
...
summary={'model':MODEL,'model_id':MODEL_ID,'blind24_sha256':bsha,'n':len(rows),'valid':len(valid),
