...
EXPECTED='9232b50c27af89dfdf71beb53d58c438c3cd7158ea852938bd0c35112a7d6c58'
here=Path(__file__).parent
packed=(here/'BLIND27_PACKED.b85').read_text(encoding='utf-8').strip()
raw=zlib.decompress(base64.b85decode(packed.encode('ascii')))
bsha=hashlib.sha256(raw).hexdigest()
if bsha!=EXPECTED: raise SystemExit(f'BLIND27 HASH MISMATCH {bsha} != {EXPECTED}')
bpath=here/'BLIND27_FROZEN.json'; bpath.write_bytes(raw)
...
for it in bench['items']:
    ...
    body={'model':MODEL,'messages':[{'role':'system','content':system},{'role':'user','content':prompt}],
          ...
          'options':{'temperature':0,'seed':2710,'num_predict':8,'top_p':1.0}}
    t=time.time()
...
summary={'model':MODEL,'model_id':MODEL_ID,'blind27_sha256':bsha,'n':len(rows),'valid':len(valid),
